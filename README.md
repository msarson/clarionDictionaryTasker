...
- **Search and replace fields (Bruce's new drivers)** — dictionary-wide match-and-replace for the pipe-separated **External Name** (Label + Extended Attributes) on fields. Match by data type (multi-select), field label regex, table regex, and current External Name regex. Replace rules cover the base name (Leave / Use Label / `PREFIX:Label` / `prefix_label` lowercase / Clear / Custom template with `{label}`, `{prefix}`, `{type}` tokens) plus any number of attribute **Set / Remove** rows — preloaded with the full object-based + traditional attribute catalogue (`BINARY`, `BOOLEAN`, `CASE`, `CHECK`, `CHECKFORNULL`, `COLLATE=`, `DEFAULT=`, `ISIDENTITY`, `NOCASE`, `NOTNULL`, `NOWHERE`, `READONLY`, `REQ`, `SELECTNAME=`, `UPDATENAME=`, `INSERTNAME=`, `SQLTYPE=`, `UUID4`, `UUID7`, `UUID8`, `VALIDATE(…)`, `WATCH`, range-comparators `> n` / `>= n` / `< n` / `<= n` / `<> n`, plus a Custom slot). **Additive** mode keeps existing tokens and overlays Set/Remove; **Rewrite** mode drops everything not explicitly Set. Per-field Before/After preview; `.DCT` backed up first.
- **SQL Migration** — launcher tile (alongside Browse / Copy fields / Copy keys / Tools) that flips a batch of tables onto a SQL driver in one pass. Pick a target driver (MSSQL / MSSQL2 / ODBC / ADO / SQLite / SQLite2 / Oracle / TOPSPEED / Btrieve / Clarion / Basic / DOS / ASCII — or type a custom value), set `DriverOptions` (default `!glo:driveroptions`) and `OwnerName` (default `!glo:owner`), optionally copy each table's Label into its `FullPathName` with a schema prefix (`dbo.`), and three-state-toggle `Create`, `Threaded`, `Encrypt`, and `Bindable` attributes (Leave alone / Turn ON / Turn OFF). Preview the before/after plan, then apply with a `.DCT` backup. The dialog honours the exclude-aliases flag and has one-click **Check TPS** to mark every topspeed row in the list.
...
| `SearchReplaceFieldsDialog.cs` | Dictionary-wide match-and-replace of the pipe-separated External Name (Label + extended attributes) on fields. |
